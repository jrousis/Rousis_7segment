...
#ifndef Rousis7segment_LIBRARY
#define Rousis7segment_LIBRARY

#include <Arduino.h>

class Rousis7segment {
public:
  enum {NO_PIN=255};

  Rousis7segment(byte n, byte SDI, byte CLK, byte LE, byte OE);
  Rousis7segment(byte n, byte SDI, byte CLK, byte LE);

  void print(const char* s, bool Invert);
  ...
  void TestSegments(uint8_t digits);
...
private:
  ...
  //  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 18: Control characters
    ...
    0x49, 0x51, 0x66, 0x28, 0x10, 0x04, 0x00, 0x2c, // 28:   ( ) * + , - . /   (0x63)
	0x7b, 0x30, 0x6d, 0x75, 0x36, 0x57, 0x5f, 0x70, // 30:   0 1 2 3 4 5 6 7
   //0b11101110,0b00000110,0b01111100,0b00111110,0b10010110,0b10111010,0b11111010,0b00100110, //(Is the old rousis fonts
